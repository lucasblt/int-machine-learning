...
#Dispose the data from the data frame into the standard form X-Y
X = raw_data[:,1:-1]
y = raw_data[:,-1]
...
pc_c = np.vstack((np.around(rhoc*100,decimals=4),np.around(np.cumsum(100*rhoc),decimals=4))).T
pc_s = np.vstack((np.around(rhos*100,decimals=4),np.around(np.cumsum(100*rhos),decimals=4))).T
